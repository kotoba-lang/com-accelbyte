(ns accelbyte.main
  "Kotodama WASM entrypoint for the Accelbyte clean-room actor (L4) — Clojure port.
  ...
  facts (`accelbyte.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "accelbyte")
(def tier "L4")
...
(defn as-int-kernel
  ...
  of accelbyte.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"accelbyte.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "accelbyte-compat" :tier tier :entities entities} 200])
...
(defn start! [] :accelbyte-compat/ready)
